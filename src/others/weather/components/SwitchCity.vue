<template>
  <div class="scbody">
    ...
    <div class="sc-city">
      ...
      <div class="city-list">
        <span 
          ...
          :key="index" 
          class="sc-hot-city" 
          @click="getWeather(item)">{{ item.name }}</span>
      </div>
        ...
        <div class="alpha-city-list">
          <template v-for="(item, index) in filteredAlphaCityList">
            <div :key="index">
              <div 
                :id="item.char" 
                class="alpha-title">{{ item.char }}</div>
              <div class="sc-line-location"/>
              <template v-for="(child, childindex) in item.list">
                <div :key="`${index}` + `${childindex}`">
                  <div 
                    class="alpah-city-text" 
                    @click="getWeather(child)">{{ child.name }}</div>
                  <div class="sc-line-location"/>
                </div>
              </template>
            </div>
    ...
    <div class="alpha-filter">
      <div 
        class="filter-hot-text" 
        @touchstart="jump">热门</div>
      <div 
        @touchmove.prevent="jump" 
        @touchend.prevent="finishFiler">
        <template v-for="(char, index) in chars">
          <div 
            :key="index" 
            class="filter-char-box">
            <span 
              v-if="current_filterchar == char" 
              class="scale-char">
              <span class="scale-char-text">{{ current_filterchar }}</span>
            </span>
            <div 
              :class="`alpha-filter-char ${current_filterchar == char ? `active-filter-char` : ``}`" 
              @touchstart="jump">{{ char }}</div>
          </div>
        </template>
      </div>
    </div>
