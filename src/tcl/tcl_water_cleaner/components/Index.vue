    ...
    <template v-if="success">
      ...
      <div
        ...
        <div
          v-if="hasTDS"
          class="record_panle"
          @click="tdsModalVisible = true">
          <div class="circle">
            <span
              v-for="i in 4"
              v-show="i==(level>4?4:level)"
              :key="i"
              :class="'c'+i"/>
          </div>
          <div
        ...
        <!-- 告警按钮-->
        <div
          v-if="alertModel.length>0"
          class="alertButton"
          @click = "goAlertpage('water_cleaner_error')">
          <div class="outer">
            <div class="inner">
              <img src="~@lib/components/assets/btn_airouter_alarm_white.png" >
              <p
                v-show="alertModel.length>=2"
                class="errorCount">{{ alertModel.length }}</p>
            </div>
          </div>
        </div>
      </div>
