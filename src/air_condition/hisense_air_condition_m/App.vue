  ...
  methods: {
    ...
    touchend(){
      ...
      if (this.isMove) {
        ...
        console.log(this.itemTemp,'this.itemTempthis.itemTempthis.itemTemp')
        
        if (this.itemTemp == 320 && this.deviceAttrs.speed == 'low' && this.deviceAttrs.mode == 'cold') {
          this.setTemperatureDis = false
    ...
    setSpeed(speed, val) {
      ...
      if (this.deviceAttrs.temperature == 320 && speed == 'low' && this.deviceAttrs.mode == 'cold') {
        return HdSmart.UI.toast('低风、制冷模式下不支持此温度，请调整后重试')
      }
    ...
    showSpeed() {
      ...
      if (this.deviceAttrs.mode=='dehumidify') {
        return HdSmart.UI.toast('除湿模式无法调节风速')
      }
