...
export default {
  ...
  methods: {
    ...
    touchend(){
      ...
      if (this.deviceAttrs.mode=='auto'||this.deviceAttrs.mode=='dehumidify'||this.deviceAttrs.mode=='wind') {
          return argv_is_mock ? HdSmart.UI.toast('该模式不支持温度调节', null, true) : HdSmart.UI.toast('该模式不支持温度调节')
      }
    ...
    showSpeed() {
      ...
      if (this.deviceAttrs.strong_wind=='on') {
        return argv_is_mock ? HdSmart.UI.toast('强力模式下不可调节风速', null, true) : HdSmart.UI.toast('强力模式下不可调节风速')
      }
