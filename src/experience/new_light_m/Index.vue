    ...
    mounted(){
      ...
      this.$nextTick(() => {
        ...
        this.$refs.canvas.addEventListener(on.end,()=> {
          ...
          this.deviceAttrs.level = parseInt(this.brightness*2.55)
        }, false)
      })
    },
    methods: {
      ...
      setSpeed(val) {
        ...
        this.deviceAttrs.temperature = val
      },
      setSwitch() {
        ...
        this.deviceAttrs.switch_status = switchStatus
        this.draw(`${this.deviceAttrs.level/255}`)
      },
      // 显示模式弹框
      showMode() {
      ...
      setModel(attr) {
        ...
        this.deviceAttrs.temperature = attr
        // .then((res) => {
        //   if(res.code == 0) {
            if(this.$refs.model.show) this.$refs.model.show = false
        //   }
