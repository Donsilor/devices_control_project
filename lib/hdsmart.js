(function() {
    ...
    sdk.Device = {
        ...
        control: function(options, success, fail) {
            ...
            var channel = opt.channel || CHANNEL_DICTIONARY[opt.method] || "REQ_AUTO";
            var options = getJSBridgeVersion() ? { channel: channel, content: opt } : opt;

            bridge.sendEvent(apiList.device_control, options, function(data) {
                if (data.msg == "TcpClient send msg success") {
    ...
    sdk.UI = {
        ...
        setNavigationBarRight: function(options) {
            ...
            if (window.WebViewJavascriptBridge) {
                WebViewJavascriptBridge.registerHandler("setNavigationBarRight_Callback", callback);
            }
        },
        ...
        goDeviceDetail: function() {
            bridge.sendEvent(apiList.goDeviceDetail, { device_uuid: window.device_uuid });
        }
    };
