(function() {
    ...
    /**
     * iOS jsbridge实现 加入flag是为了防止在虚拟体验特殊场景下不能调用popwindow的原生接口
     */
    function iosCall(method, data, callback, empty, flag) {
        console.log('调用iosSDK')
        ...
        if(window.JSBridgeEventHandler && data.params && data.params.eventCode === 'remoteControlEvent'){
          ...
          if (flag && window.JSBridgeEventHandler) {
            fetchDataObj = JSBridgeEventHandler
          } else {
        ...
        console.log(new Date().getHours()+':'+new Date().getMinutes()+':'+new Date().getSeconds()+'---' + method + ' = ' + JSON.stringify(data))

        fetchDataObj.callNativeFunction(method, data, function(response) {
          console.log(new Date().getHours()+':'+new Date().getMinutes()+':'+new Date().getSeconds()+'---' + 'iOS返回的状态=====>', response)
            callback(response)
        })
    }

    /**
     * android jsbridge实现 加入flag是为了防止在虚拟体验特殊场景下不能调用popwindow的原生接口
     */
    function androidCall(method, data, callback, toJSON, flag) {
      ...
      if (window.WebViewJavascriptBridge && !argv_is_mock || (window.WebViewJavascriptBridge && data.params && data.params.eventCode === 'remoteControlEvent')) {
          ...
          window.WebViewJavascriptBridge.callHandler(method, data, function(
              ...
              console.log(new Date().getHours()+':'+new Date().getMinutes()+':'+new Date().getSeconds() +'---'+ method + ' res = ' + response)
              if (toJSON) {
          ...
          if (flag && window.WebViewJavascriptBridge) {
            if (typeof method === 'string') {
              method = method.split('.').pop()
            }
            window.WebViewJavascriptBridge.callHandler(method, data, function(
    ...
    var apiList = {
        ...
        goDeviceRoom: 'goDeviceRoom', //跳转设备房间
        goDeviceInfo: 'goDeviceInfo', //跳转设备信息
        goDeviceHelp: 'goDeviceHelp', //跳转使用帮助
        goHomePage: 'goHomePage', //跳转APP首页   参数 type '1' 解绑跳首页    '0' 普通跳首页
        syncName: 'syncName', //同步名称给APP
    }
        ...
        getNetworkStatus: function(success, fail){
          var isTimeout = false
            ...
            var callback = function(data) {
                ...
                clearTimeout(timer)
                if (typeof Number(data) === 'number' && Number(data)%1 === 0) {
                    success(data)
    ...
    sdk.UI = {
         ...
         * toast提示
         */
        toast: function(message, duration, mock) {
        ...
        popWindow: function(mock=false) {
            if (!mock) {
              bridge.sendEvent(apiList.popWindow)
            } else {
              bridge.sendEvent(apiList.popWindow, {}, () => {}, {}, true)
            }
        },
        ...
        vibrate: function(mock=false) {
          if (!mock) {
            bridge.sendEvent(apiList.vibrate)
          } else {
            bridge.sendEvent(apiList.vibrate, {}, () => {}, {}, true)
          }
        },
        goDeviceRoom: function() {
          bridge.sendEvent(apiList.goDeviceRoom)
        },
        goDeviceInfo: function() {
          bridge.sendEvent(apiList.goDeviceInfo)
        },
        goDeviceHelp: function() {
          bridge.sendEvent(apiList.goDeviceHelp)
        },
        goHomePage: function(type) {
          bridge.sendEvent(apiList.goHomePage, {
            type: type
          })
        },
        syncName: function(name) {
          bridge.sendEvent(apiList.syncName, {
            name: name
          })
        },
    ...
    sdk.network={
       httpRequest(method, param, callBack){
         bridge.sendEvent(method, param, function (res) {
    ...
    window.onDeviceChange = function(data) {
        console.log(new Date().getHours()+':'+new Date().getMinutes()+':'+new Date().getSeconds()+'---' + 'device change = ' + JSON.stringify(data))
