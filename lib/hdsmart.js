(function() {
    ...
    function iosCall(method, data, callback) {
        if (!window.mock && !window.WebViewJavascriptBridge) {
          return
        }
        ...
        var fetchDataObj = {}
        if(argv_is_mock && window.mock){
          fetchDataObj = window.mock
        } else {
          fetchDataObj = JSBridgeEventHandler
        }
        fetchDataObj.callNativeFunction(method, data, function(
            response
        ) {
    ...
    function androidCall(method, data, callback, toJSON) {
        ...
        if (window.WebViewJavascriptBridge && !argv_is_mock) {
    ...
    sdk.Device = {
        ...
        control: function(options, success, fail) {
            ...
            var options = getJSBridgeVersion()
                ? { channel: channel, content: opt }
                : opt
