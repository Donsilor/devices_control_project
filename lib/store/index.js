  ...
  actions: {
    ...
    getDeviceInfo({ commit,state }) {
      ...
      return new Promise((resolve, reject) => {
        HdSmart.Device.getSnapShot(res => {
          ...
          let newState = Object.assign({}, state.deviceAttrs, res)
          commit('setDeviceInfo', newState)
    ...
    addListener({ commit, state }) {
      HdSmart.onDeviceStateChange(res => {
        commit('toggleState')
        let newState = Object.assign({}, state.deviceAttrs, res.result)
        commit('setDeviceInfo', newState)
        if (window.device_uuid) {
          window.localStorage.setItem(window.device_uuid, JSON.stringify(res.attribute))
        }
